...
export function BlogPostForm({ initial }: BlogPostFormProps) {
  ...
  async function importMarkdown() {
    ...
    // Ask the AI to generate title, excerpt and tags from the imported content.
    ...
    try {
      ...
      if (data?.title) {
        ...
        if (!slugTouched) setSlug(slugify(data.title as string));
      }
      ...
      toast.success("Markdown imported and details generated by AI", {
        id: toastId,
      });
    } catch (err) {
      toast.error(
        err instanceof Error
          ? `Imported, but AI details failed: ${err.message}`
          : "Imported, but AI details failed",
        { id: toastId },
      );
    }
  }
