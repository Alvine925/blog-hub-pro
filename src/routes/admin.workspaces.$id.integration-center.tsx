...
import {
  ...
  Sparkles, RefreshCw, FileText, Code2,
} from "lucide-react";
...
export const Route = createFileRoute("/admin/workspaces/$id/integration-center")({
  head: () => ({ meta: [{ title: "Integration Center" }] }),
...
const CMS_BASE_URL =
  typeof window !== "undefined"
    ? `${location.origin.includes("localhost") ? "https://your-project.supabase.co/functions/v1/content-router" : location.origin + "/functions/v1/content-router"}`
    : "https://your-project.supabase.co/functions/v1/content-router";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          The prompt will be tailored for this tool's agent workflow.
        </p>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Determines when and where your content is fetched.
        </p>
      </div>
...
function StepGenerate({
  ...
  function download(ext: "md" | "txt") {
    ...
    a.href     = url;
    a.download = `lunar-cms-${fw}-integration.${ext}`;
    a.click();
  ...
  function handleNext() {
    if (step === 5) {
      ...
      const sel: GeneratorSelections = {
        ...
        stylingId:        styling,
        apiBaseUrl:       CMS_BASE_URL,
      };
      const result = generatePrompt(sel);
      setOutput(result);
      setStep(6);
    } else {
      setStep((s) => s + 1);
    ...
    <div className="min-h-full px-8 py-8 space-y-8">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Generate a tailored AI implementation prompt to integrate your website with the Lunar CMS API — no manual coding required.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-white shadow-sm">
        <div className="p-8">
          ...
          {step === 2 && (
            <StepAiPlatform selected={aiPlatform} onSelect={setAiPlatform} />
          )}
          {step === 3 && (
            <StepContent selected={content} onToggle={toggleContent} />
          )}
          {step === 4 && (
            <StepRendering
              selected={rendering}
          ...
          {step === 5 && (
            <StepStyling selected={styling} onSelect={setStyling} />
          )}
          {step === 6 && output && (
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {[
            { icon: "🔑", title: "API Key Required", desc: "Make sure you have an API key from the API Keys section before integrating." },
            { icon: "⚡", title: "One Prompt, Full Integration", desc: "The generated prompt instructs the AI agent to build the entire integration automatically." },
            { icon: "🔒", title: "Read-Only & Secure", desc: "Publishable keys expose only published content. Your CMS data stays safe." },
          ].map((card) => (
