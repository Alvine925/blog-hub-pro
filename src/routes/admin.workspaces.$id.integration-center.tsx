...
function getDefaultApiUrl(): string {
  ...
  if (typeof window !== "undefined" && !location.origin.includes("localhost")) {
    return `${location.origin}/functions/v1/content-router`;
  }
  return "https://your-project.supabase.co/functions/v1/content-router";
}
    ...
    <div className="min-h-full px-8 py-8 space-y-8">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Generate a tailored AI prompt to build an external blog with likes, comments, share modals, and the Lunar CMS attribution banner — all wired automatically.
          </p>
        </div>
      </div>
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {[
            ...
            { icon: "💬", title: "Engagement Built-in",    desc: "Likes, comments, share modals, view tracking, and the Powered by Lunar CMS banner are included automatically in the prompt." },
            { icon: "🔒", title: "Read-Only & Secure",     desc: "Publishable keys expose only published content. Engagement writes proxy through the server so the API key never reaches the browser." },
          ].map((card) => (
