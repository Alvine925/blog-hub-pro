    ...
    <div className="min-h-full px-4 py-4 sm:px-8 sm:py-8">
      ...
      {[...Array(10)].map((_, i) => (
        <div key={i} className="flex items-center gap-3 border-b border-border py-3 last:border-0">
          ...
          <Skeleton className="h-4 w-20 shrink-0 rounded-full hidden lg:block" />
          <Skeleton className="h-3 w-20 shrink-0 hidden xl:block" />
...
import {
  ...
  Eye, Heart, MessageSquare, Sparkles, CheckSquare, Square, Loader2, Download,
} from "lucide-react";
...
function WorkspaceNews() {
  ...
  function handleExport() {
    exportToCsv(exportFilename("news"), news.map((n) => ({
      Title:        n.title,
      Status:       n.status,
      Category:     n.category || "",
      Excerpt:      n.excerpt  || "",
      ...
      Views:        batchStats?.[n.id]?.views    ?? 0,
      Likes:        batchStats?.[n.id]?.likes    ?? 0,
      Comments:     batchStats?.[n.id]?.comments ?? 0,
    })));
  }
