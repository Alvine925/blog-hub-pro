      ...
      {news.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-20 border-y border-border text-center">
          ...
          <p className="text-sm text-muted-foreground">
            No news yet. It's generated automatically when "News" is selected during onboarding.
          </p>
        </div>
      ) : (
        <div>
          ...
          {news.map((item) => (
            ...
            >
              ...
              <span className={cn("w-20 shrink-0 text-xs hidden lg:block", STATUS_STYLE[item.status])}>
                {item.status}
              </span>
