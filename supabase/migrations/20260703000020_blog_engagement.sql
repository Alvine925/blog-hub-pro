...
CREATE TABLE IF NOT EXISTS public.blog_likes (
  ...
  visitor_id   TEXT        NOT NULL,   -- anonymous fingerprint (client UUID or IP+UA hash)
...
CREATE TABLE IF NOT EXISTS public.blog_comments (
  ...
  name         TEXT        NOT NULL,
  email        TEXT,                  -- private — never returned in public API
  website      TEXT,
...
-- ── 3. blog_views (detailed tracking for analytics) ───────────────────────────
...
-- Dedup index: same visitor cannot record more than one view per post per 4h window
-- (enforced at app layer using created_at > now() - interval '4 hours')
...
-- ── 4. blog_posts — add word_count ────────────────────────────────────────────
ALTER TABLE public.blog_posts
  ADD COLUMN IF NOT EXISTS word_count INTEGER;

-- ── 5. workspaces — add engagement feature flags + branding ───────────────────
...
-- blog_likes: public read, service role write (engagement function uses service role)
...
-- blog_comments: public read approved, service role full access
